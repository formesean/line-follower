...
void Sensors::setTypeRC()
{
    _type = Type::RC;
    _maxValue = _timeout;
}

void Sensors::setTypeAnalog()
{
    _type = Type::Analog;
    _maxValue = 1023; // Arduino analogRead() returns a 10-bit value by default
}

void Sensors::setSensorPins(const uint8_t *pins, uint8_t sensorCount)
{
...
void Sensors::calibrate(ReadMode mode)
{
    if (mode == ReadMode::Manual)
    {
        return;
    }

    if (mode == ReadMode::On || mode == ReadMode::OnAndOff)
    {
        calibrateOnOrOff(calibrationOn, ReadMode::On);
    }
    else if (mode == ReadMode::OddEven || mode == ReadMode::OddEvenAndOff)
    {
        calibrateOnOrOff(calibrationOn, ReadMode::OddEven);
    }

    if (mode == ReadMode::OnAndOff || mode == ReadMode::OddEvenAndOff || mode == ReadMode::Off)
    {
        calibrateOnOrOff(calibrationOff, ReadMode::Off);
    }
}

void Sensors::calibrateOnOrOff(CalibrationData &calibration, ReadMode mode)
{
...
void Sensors::read(uint16_t *sensorValues, ReadMode mode)
{
    switch (mode)
    {
    case ReadMode::Off:
...
void Sensors::readCalibrated(uint16_t *sensorValues, ReadMode mode)
{
...
void Sensors::readPrivate(uint16_t *sensorValues, uint8_t start, uint8_t step)
{
    if (_sensorPins == nullptr)
    {
        return;
    ...
    default: // Type::Undefined or invalid - do nothing
        return;
    }
}

uint16_t Sensors::readLinePrivate(uint16_t *sensorValues, ReadMode mode, bool invertReadings)
{
    bool onLine = false;
